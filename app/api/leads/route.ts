...
import { bulkLeads, createLead, listLeads } from "@/lib/workspace-store";
...
export async function GET(request: Request) {
  try {
    ...
    return json(await listLeads(user.id, scope));
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    return json(await createLead(user.id, body), 201);
  } catch (error) {
    return errorResponse(error);
  }
}
...
export async function PATCH(request: Request) {
  try {
    ...
    return json(await bulkLeads(user.id, parseIdList(body.ids), action));
  } catch (error) {
    return errorResponse(error);
  }
}
